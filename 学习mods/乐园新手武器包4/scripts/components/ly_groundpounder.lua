local LyGroundPounder = Class(function(self, inst)
    self.inst = inst
...
function LyGroundPounder:GetPoints(pt)
    local points = {}
...
function LyGroundPounder:DestroyPoints(points, breakobjects, dodamage)
            ...
            if #ents > 0 then
                if breakobjects then
                    for i, v2 in ipairs(ents) do
                        if v2 ~= self.inst and v2:IsValid() and (self.canhitFn == nil or self.canhitFn(self.inst,v2)) then
...
function LyGroundPounder:GroundPound(pt)
...
function LyGroundPounder:GroundPound_Offscreen(position)
    self.inst.components.combat:EnableAreaDamage(false)
    ...
    for i, v in ipairs(ents) do
        if v ~= self.inst and v:IsValid() and not v:IsInLimbo() and (self.canhitFn == nil or self.canhitFn(self.inst,v)) then
