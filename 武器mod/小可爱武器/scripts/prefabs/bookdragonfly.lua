local assets =
{
Asset("ANIM", "anim/bookdragonfly.zip"),
	...
	reader:StartThread(function ()
		for k = 1, numboss do
			...
			if result_offset ~= nil then
				...
				local boss = SpawnPrefab("dragonfly")
...
local function MakeBook(def)
	...
	for i, v in ipairs(book_defs) do
		...
		local function fn()
			...
			inst.AnimState:SetBank("bookdragonfly")
            inst.AnimState:SetBuild("bookdragonfly")
	        ...
	        inst.components.inventoryitem.atlasname = "images/inventoryimages/bookdragonfly.xml"
