local assets=
{
Asset("ANIM", "anim/hnd.zip"),
Asset("ANIM", "anim/dshnd.zip"),
...
local function onequip(inst, owner)
	...
	owner.AnimState:OverrideSymbol("swap_object", "hnd", "hnd")
...
local function onattack(inst, attacker, target, skipsanity, owner)
		...
		if target:HasTag("deerclops") and target.components.health:IsDead() then--巨鹿
	...
	if  target ~= nil and target.components.freezable ~= nil then
			target.components.freezable:AddColdness(math.ceil(0.1+0.01*inst.components.redcherrystatus.level))
		end
...
local function fn()
	...
	inst.components.inventoryitem.atlasname = "images/inventoryimages/hnd.xml"
	...
	inst.components.named:SetName("寒羽之刃\n击杀巨鹿和犀牛可以升级")
...
return Prefab("common/inventory/hnd", fn, assets, prefabs)
